#!/usr/bin/env tsx
/**
 * Automatic relay: Solana Devnet → Fogo Testnet via Executor
 */
...
async function pollStatus(txHash: string, srcChain: string = 'Solana', timeout = 180000) {
...
async function main() {
    const greeting = process.argv[2] || 'Auto-relay Solana→Fogo! 🚀';
    console.log('🚀 Auto-Relay: Solana Devnet → Fogo Testnet\n');
    ...
    const chainBuf = Buffer.alloc(2); chainBuf.writeUInt16LE(CHAIN_ID_FOGO);
    ...
    const quote = await getQuote(CHAIN_ID_SOLANA, CHAIN_ID_FOGO, 200000);
    ...
    const dstChainBuf = Buffer.alloc(2); dstChainBuf.writeUInt16LE(CHAIN_ID_FOGO);
    ...
    const result = await pollStatus(sig);
    
    if (result?.success) {
        console.log('\n\n🎉 SUCCESS! Message relayed to Fogo!');
        console.log('Fogo TX:', result.data.txs?.[0]?.txHash || 'check explorer');
    } else if (result) {
